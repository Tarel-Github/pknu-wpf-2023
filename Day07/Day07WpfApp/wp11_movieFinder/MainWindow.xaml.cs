...
/*            foreach (MovieItem item in GrdResult.SelectedItems)
        ...
        private async void BtnViewFavorite_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Commons.connString))
                {
                    if(conn.State == ConnectionState.Closed) conn.Open();   // 서버 닫혀있으면 열고
                    var query = @"SELECT id
                                       , Title
                                       , Original_Title
                                       , Release_Date
            ...
            try  // 삭제
            {
                using(SqlConnection conn = new SqlConnection(Commons.connString))
                {
                    ...
                    if(delRes == GrdResult.SelectedItems.Count)
                    {
                        ...
                        StsResult.Content = $"즐겨찾기 {delRes}건 조회완료";
                    }
                    else
                    {
                        await Commons.ShowMessageAsync("삭제", "DB삭제 일부 성공!!");   // 발생 가능성 거의 없음
                    }
                }
            }
            catch(Exception ex)
